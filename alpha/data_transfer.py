...
import pandas as pd
# 将股票交易明细表转换为估值表
data_2016_11=pd.read_excel('chengye\stock.xlsx').sort_values(u'成交时间')
...
data=pd.DataFrame(data.values,columns=['name','code','deal_amount','sell_or_buy','date','deal_values'],index=data.index) # 取出[u'证券名称',u'证券代码',u'成交数量',u'委托方向',u'业务日期',u'成交金额']列，并转换为['name','code','deal_amount','sell_or_buy','date','deal_values']
